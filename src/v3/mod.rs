...
impl CudnnConvBwdFilterOp {
  ...
  pub fn create_deterministic(src_desc: CudnnTensorDesc<f32>, diff_desc: CudnnTensorDesc<f32>, conv_desc: CudnnConvDesc, grad_filter_desc: CudnnFilterDesc<f32>, grad_bias_desc: CudnnTensorDesc<f32>, handle: &CudnnHandle) -> CudnnResult<CudnnConvBwdFilterOp> {
    let algo = cudnnConvolutionBwdFilterAlgo_t::Deterministic;
    let mut work_size: size_t = 0;
    let status = unsafe { cudnnGetConvolutionBackwardFilterWorkspaceSize(
        ...
        grad_filter_desc.ptr,
        algo,
        &mut work_size as *mut _,
    ) };
    new_result(CudnnConvBwdFilterOp{
      algo: algo, work_size: work_size as usize, time_ms: 0.0,
      src_desc: src_desc,
      diff_desc: diff_desc,
      conv_desc: conv_desc,
      grad_filter_desc: grad_filter_desc,
...
impl CudnnConvBwdDataOp {
  pub fn create_fastest(filter_desc: CudnnFilterDesc<f32>, diff_desc: CudnnTensorDesc<f32>, conv_desc: CudnnConvDesc, grad_data_desc: CudnnTensorDesc<f32>, handle: &CudnnHandle) -> CudnnResult<CudnnConvBwdDataOp> {
    let mut count: c_int = 0;
    ...
    new_result(CudnnConvBwdDataOp{
      ...
      diff_desc: diff_desc,
      conv_desc: conv_desc,
      grad_desc: grad_data_desc,
    }, status)
  }

  pub fn create_deterministic(filter_desc: CudnnFilterDesc<f32>, diff_desc: CudnnTensorDesc<f32>, conv_desc: CudnnConvDesc, grad_data_desc: CudnnTensorDesc<f32>, handle: &CudnnHandle) -> CudnnResult<CudnnConvBwdDataOp> {
    let algo = cudnnConvolutionBwdDataAlgo_t::Deterministic;
    let mut work_size: size_t = 0;
